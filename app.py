...
class FocusGuardProcessor(VideoProcessorBase):
    ...
    def recv(self, frame):
        ...
        # Inisialisasi MediaPipe di dalam recv (Lazy Loading) agar aman di server Linux Cloud
        with mp.solutions.face_mesh.FaceMesh(
            ...
            cv2.putText(img, f"Status: {status_text}", (20, 50), 
                        cv2.FONT_HERSHEY_SIMPLEX, 0.8, color, 2, cv2.LINE_AA)

        return frame.from_ndarray(img, format="bgr24")

# --- WEBRTC STREAMER ---
# Konfigurasi STUN & TURN Server publik lengkap agar tidak terhalang NAT / Firewall Cloud
...
webrtc_streamer(
    ...
    async_processing=True # Mengaktifkan pemrosesan asinkron untuk kestabilan cloud
)